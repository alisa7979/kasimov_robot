...
const int LINE_THRESH_FL = 735;
const int LINE_THRESH_FR = 860;
const int LINE_THRESH_RL = 860;
const int LINE_THRESH_RR = 850;

// [라인 센서 극성] 경계 테이프 위에서 값이 바닥보다 크면 true — 실측으로 확인됨 (빨강이 항상 더 큼)
const bool LINE_READS_HIGHER = true;

// [공격 및 추적 설정]
const int RUSH_DISTANCE = 10;       // 라인을 무시하고 돌진할 거리 (cm) — 3은 초음파 최소거리 탓에 사실상 도달 불가
const unsigned long CONTACT_LATCH_MS = 2000; // 근접 감지 후 이 시간 동안은 접촉(밀기) 상태로 간주
...
const unsigned long START_DELAY_MS = 1000; // 대회 규정의 시작 대기 시간 확인 후 맞추세요 (보통 3~5초)
