...
const int SEARCH_SPEED = 150;       // 탐색 회전 시 속도

// [시간 설정 (밀리초 단위, 1000 = 1초)]
// 테스트를 통해 180도와 45도 회전에 딱 맞는 시간을 입력하세요.
const unsigned long TIME_BACK_1MS = 1000;
const unsigned long TIME_TURN_180 = 800;
const unsigned long TIME_FORWARD_2MS = 2000;
const unsigned long TIME_TURN_45 = 250;
 ...
 // 2. 비동기 예외 및 탈출 상태 머신 처리 (delay 방지)
   if (currentState != STATE_NORMAL) {
    ...
    if (currentState == STATE_ESCAPE_REAR) {
      if (isLine(lineFrontLeft) || isLine(lineFrontRight)) {
...
bool checkLineSensors() {
  ...
  if (fl && fr) {
    ...
    stateStartTime = millis();
    stateDuration = TIME_FORWARD_2MS;
